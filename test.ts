...
} from "./gasBalanceMonitorService";
import { WebhookService, getWebhookService } from "./webhook";

// ─── Helpers ─────────────────────────────────────────────────────────────────

const mockLoadAccount = (balance: string) => {
  const server = new (Horizon.Server as jest.MockedClass<
    typeof Horizon.Server
  >)("");
...
const mockLoadAccountFailure = () => {
  const server = new (Horizon.Server as jest.MockedClass<
    typeof Horizon.Server
  >)("");
  (server.loadAccount as jest.Mock).mockRejectedValue(
