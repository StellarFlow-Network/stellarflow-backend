    ...
    async submitTransactionWithRetries(builderFn, maxRetries = this.MAX_RETRIES, baseFee) {
        ...
        while (attempt <= maxRetries) {
            try {
                ...
                const nextSequence = await sequenceManager.getNextSequence(this.getKeypair().publicKey());
                const sourceAccount = new Account(this.getKeypair().publicKey(), nextSequence);
