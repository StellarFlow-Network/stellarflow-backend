    ...
    try {
        ...
        res.json({
            ...
            data: {
                ...
                signers: multiSigPrice.multiSigSignatures?.map((sig) => ({
 ...
 */
router.get("/multi-sig/pending", async (req, res) => {
    try {
...
router.get("/multi-sig/:multiSigPriceId/signatures", async (req, res) => {
    try {
        ...
        const signatures = await multiSigService.getSignatures(parseInt(multiSigPriceId, 10));
        res.json({
            success: true,
            data: {
...
router.post("/multi-sig/:multiSigPriceId/record-submission", async (req, res) => {
    try {
        ...
        await multiSigService.recordSubmission(parseInt(multiSigPriceId, 10), memoId, stellarTxHash);
        res.json({ success: true });
    }
    catch (error) {
 ...
 */
router.get("/multi-sig/signer-info", async (req, res) => {
    try {
        const signerInfo = multiSigService.getLocalSignerInfo();
        res.json({
            success: true,
...
//# sourceMappingURL=priceUpdates.js.map