...
export function apiKeyAuth() {
    return async function (req, res, next) {
        ...
        if (!rawKey || typeof rawKey !== "string" || rawKey.trim() === "") {
            sendError(res, 401, "MISSING_API_KEY", "Request must include a valid X-API-Key header.");
            return;
        }
        // ── 2. Resolve the required scope ───────────────────────────
        const required = requiredScopeForMethod(req.method);
        if (required === null) {
            sendError(res, 405, "METHOD_NOT_ALLOWED", `HTTP method "${req.method}" is not supported.`);
            return;
        }
        ...
        try {
            ...
            sendError(res, 503, "SERVICE_UNAVAILABLE", "Authentication service temporarily unavailable.");
            return;
        }
        // ── 4. Key not found ─────────────────────────────────────────
        if (!apiKeyRecord) {
        ...
        if (apiKeyRecord.expiresAt && apiKeyRecord.expiresAt < new Date()) {
            sendError(res, 403, "API_KEY_EXPIRED", `This API key expired on ${apiKeyRecord.expiresAt.toISOString()}.`);
            return;
        }
        // ── 7. Scope check ───────────────────────────────────────────
        if (!hasScope(apiKeyRecord.scopes, required)) {
            sendError(res, 403, "INSUFFICIENT_SCOPE", `This endpoint requires the "${required}" scope. ` +
...
function scopeGuard(scope) {
    return (_req, res, next) => {
        ...
        if (!hasScope(key.scopes, scope)) {
            sendError(res, 403, "INSUFFICIENT_SCOPE", `This action requires the "${scope}" scope.`);
            return;
        }
        next();
    };
